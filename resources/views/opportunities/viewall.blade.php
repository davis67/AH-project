...
@section('content')
          <div class="row">
			...
			<form method="post" action="{{route('changeStatus')}}">
				...
				<div class=" col-md-8">
						<a href="{{ route('opportunities.create') }}" style="float:right" class="btn btn-outline-danger btn-sm pull-right"><i class="fa fa-fw fa-plus"></i>Create Opportunity</a>
				...
				<table class="table example" >
					...
					<tbody>
							...
							<td>
								@if(($opportunity->sales_stage)=== 'Closed Won' && !($opportunity->is_project))
								<a href="{{ route('projects.create', ['id'=>$opportunity->id ]) }}" class="btn btn-xs btn-outline-danger">Add Project<a>
								@elseif(($opportunity->sales_stage)=== 'Closed Won' && ($opportunity->is_project))
								<a href="{{ route('projects.show', $opportunity->project->id) }}"" class="btn btn-xs btn-outline-danger">View Project</a>
								@else
